...
Class DebugGUI
{
	...
	Pause(Text:="Press Resume to continue...") {
		...
		;OG SendMessage, 0x0028, this.gui.hResume, 1  ; WM_NEXTDLGCTL = 0x0028
		;SendMessage(0x0028, this.gui.hResume, 1) ; 0x0028=WM_NEXTDLGCTL, 1=select the WParam ControlID
		;SendMessage(0x0028, , ,this.gui.hGui ) ; 0x0028=WM_NEXTDLGCTL, 1=select the next dlg ctrl on the GuiID
	...
	SetText(pText)
	{
		...
		;OG ControlSetText,, % vText, % "ahk_id " this.gui.hEdit
		ControlSetText(this.gui.hEdit, vText)

		;ControlSend, , {End}, % "ahk_id " this.gui.hEdit
		;ControlSend, , {Home}, % "ahk_id " this.gui.hEdit
	...
	Splash(Title:="DEBUG", Text:="Note the Debug Gui", Duration:=4000, x:="Center", y:="Center", w:=180, h:=140) {
		...
		;OG Depreciated so don't add to AHKEZ
	 	SplashTextOn, %w%, %h%, %Title%, %Text%
		;OG WinMove %Title%, , %x%, %y%
		hSplash := WinGetID(Title)
		WinMove(ahkid(hSplash),, x, y)
		;OG SetTimer, SplashTimerOff, %Duration%
	...
	_PauseHelp(Text:="Press HELP to continue...") {	
		this.Paste(Text)
		;SendMessage(0x0028, this.gui.hHelp, 1)
		ControlSelect(this.gui.hResume, this.gui.hGui)
		this.gui.Paused:=True		
		While (this.gui.Paused)
		{
...
Gui_Timer_Start:
	...
	_OnTick() {
		...
		if GetKeyState("Control", "P") And GetKeyState("J", "P") {
			MB("TICK")
		}
	...
	_OnResume() {
		...
		GuiControl("Hide", this.gui.hResume)
		;SendMessage, 0x0028, this.gui.hResume, 1, , % "ahk_id" this.gui.hGui  ; WM_NEXTDLGCTL = 0x0028
		;SendMessage(0x0028, this.gui.hResume, 1)
		ControlSelect(this.gui.hResume, this.gui.hGui)
	}
	...
	_WM_SYSCOMMAND(wp, lp) {
		...
		;**FIXME - test with static_Gui AHKEZ_Gui?
		Gui, %A_Gui%: DeStroy
  ...
  7CG6MJ6ppTXl5eXPoHk9iHj4cjILsNfnx43aG+Rk15NBuoTJ/oRIf27Gk8uqKf1fXl8B7KLH7kDILXMAAAAASUVORK5CYII=
	...
	ListVars(vars*){
		...
		; 	. "`n`n ex.What=" ex.What
		; 	. "`n`n Match=" Match
		; 	. "`n`n Match1=" Match1 " `n`n Match2=" Match2 "`n`n Match3=" Match3)

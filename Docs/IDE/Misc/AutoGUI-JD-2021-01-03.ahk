JD_PATCH_VERSION:="2021-01-03-01" ;so we can edit the exe with notepad++ and find this text tag
/**
2021-01-03  AutoGUI.ahk  JD_DisableKeys				Disable non-standard keys, remove catch-all at WriteFile
2021-01-02	AutoGUI.ahk  JD_GetTempFileName 	Save backups to filename_yyyymmdd_hhmmss.ahk.tmp
2020-12-25	AutoGUI.ahk  JD_FixFilename 			Catch-call to fix file ext issues
2020-12-22	AutoGUI.ahk  JD_AutoGOTO					Auto start \Tools\AutoGOTO.ahk
2020-12-21	AutoGUI.ahk  JD_F10								Disable the F10 key that I keep pressing by mistake when I reach for F9
12/09/2020	AutoGUI.ahk  JD_GetTempFileName		Save backups to yyyymmdd_hhmmss_filename.ahk.tmp versus a random number
2020-11-22	AutoGUI.ahk  JD_SaveAll						Save All, then Execute
*/
;
Global JD_DisableKeys := True
;
; Adventure v2.6.2
...
;JD_AutoGOTO
If FileExist(".\Tools\AutoGOTO.ahk")
	Run %A_AHKPath% ".\Tools\AutoGOTO.ahk"
...
/*
	JD_FixFilename:
	Issue:	Script (no ext) was saved as Script (no ext) and the Lexer is reset to plain Text
					Scriptv1.1	(no ext) was saved as Scriptv1.1 (ext .1) and the Lexer is reset to plain Text
	Fix:		Scriptv1.1	(no ext) is now saved as Scriptv1.1.ahk and the Lexer is unchanged
					Scriptv1.		same
	Note:		AutoGUI\Lib\AuxLib.ahk, Line 274:IsAhkFileExt(FileExt) appears to accept .ahk, .ah2, .ahh, ahs
					I'm not sure if this lexer supports AHK v2 .ah2 yet, so let's stick with .ahk for now		
*/	

JD_FixFileName(Filename) {
	...
	SplitPath Filename, OutFilename,, OutFileExt
  ...
  If (OutFileExt = "") {
	...
	} else If OutFileExt is digit
	{
   Filename .= ".ahk"
	}

	if (SubStr(Filename,-7)=".ahk.ahk")
		Filename := SubStr(Filename,1,-4)

	;MsgBox % "BEFORE=[" BEFORE "]`n`nAFTER=[" SelectedFile "]"
...
SaveAs(n) {
  ...
  SplitPath SelectedFile, Filename,, FileExt
	...
	;JD_FixFileName
	SelectedFile := JD_FixFileName(SelectedFile)
  SplitPath SelectedFile, Filename,, FileExt
	;JD_FixFileName
...
GetTempFileName(Dir, Ext := "tmp") {
	Local Num, Filename
	;JD_GetTempFileName
	;calling function: params=GetTempFileName(g_BackupDir, Ext:="ahk.tmp")
	FormatTime, TimeStamp, ,yyyyMMdd_HHmmss
	SelectedFile := Sci[n].Filename ; Filename.ahk	
	SplitPath SelectedFile, , , , OutNameNoExt
	Filename := Dir . "\" . OutNameNoExt . "_" . TimeStamp . "." . Ext ; Filename_YYYYMMDD_HHMMSS.ahk.tmp
	Return Filename
...
WriteFile(Filename, String, Encoding := "UTF-8") {
	...
	;May be redunant, but I'm tired of fighthing this so keep it
	;JD_WriteFile
	Filename := JD_FixFileName(Filename)
...
Execute() {
    ...
    GoSub SaveAll ; JD_SaveAll
		;Sleep, 100		; give time to write all tabs to disk
